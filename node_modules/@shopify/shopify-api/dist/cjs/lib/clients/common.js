...
function throwFailedRequest(body, atMaxRetries, response) {
    ...
    if (response.status === types.StatusCode.Ok && body.errors.graphQLErrors) {
    ...
    switch (true) {
        case response.status === types.StatusCode.TooManyRequests: {
            if (atMaxRetries) {
        ...
        case response.status >= types.StatusCode.InternalServerError:
            if (atMaxRetries) {
                throw new error.HttpMaxRetriesError('Attempted the maximum number of retries for HTTP request.');
            }
            else {
